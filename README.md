...
| **Replication end-to-end** | Layer-1 VR scenarios in `test/e2e/replication`                                                     | Create VolumeReplication/VolumeReplicationClass on a live cluster; require controller and CSI driver with replication support. See [Replication end-to-end suite](docs/testing/replication-e2e-suite.md).  |
...
- Replication end-to-end suite against existing cluster (controller + CSI driver required): `make test-replication-e2e` or `./hack/run-replication-e2e.sh`. Use `GINKGO_FOCUS` to run specific tests (e.g. `GINKGO_FOCUS="L1-E-001" ./hack/run-replication-e2e.sh`). See [Replication end-to-end suite](docs/testing/replication-e2e-suite.md).
